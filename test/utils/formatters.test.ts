...
describe("utils:eth Tx to thor Tx", () => {
  it("input without block chainTag should throw error!", () => {
    expect(() => { utils.ethToThorTx({}); }).to.throw("chainTag must be set");
  });

  it("input without gas  should throw error!", () => {
    expect(() => { utils.ethToThorTx({ chainTag: "0x9a" }); }).to.throw("gas must be more than 21000");
  });

  it("input without gas hould throw error!", () => {
    expect(() => { utils.ethToThorTx({ chainTag: "0x9a", gas: 20000 }); }).to.throw("gas must be more than 21000");
  });

  it("input without nonce hould throw error!", () => {
    expect(() => { utils.ethToThorTx({ chainTag: "0x9a", gas: 21000}); }).to.throw("nonce must be set");
  });
